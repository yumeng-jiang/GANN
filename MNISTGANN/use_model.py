...
def discriminator(bottom, z, reuse=False):
    ...
    d_out = slim.fully_connected(tf.concat(1, [slim.flatten(dis3),z]),1,activation_fn=tf.nn.sigmoid,\
...
Dx = discriminator(real_in, z_in) #Produces probabilities for real images
Dg = discriminator(Gz, z_in, reuse=True) #Produces probabilities for generator images
...
with tf.Session() as sess:  
    ...
    save_images(np.reshape(newZ[0:batch_size_sample],[len(labels),32,32]),[8,8],sample_directory+'/0_9'+'.png')